...
#define __RAYTRACE_H_
  ...
  class Raytracer {
    ...
    void upload() {
      ...
      // Instance id data
      unsigned int idVBO;
      glGenBuffers(1, &idVBO); gl_error();
      glEnableVertexAttribArray(3); gl_error();
      glBindBuffer(GL_ARRAY_BUFFER, idVBO); gl_error();
      glBufferData(GL_ARRAY_BUFFER, total_id_mem, &indices[0], GL_STATIC_DRAW); gl_error();
      glVertexAttribPointer(3, 3, GL_UNSIGNED_INT, GL_FALSE, sizeof(uint32_t), (void*)0); gl_error();
      glVertexAttribDivisor(3, 1); gl_error();
