...
#define __RAYTRACE_H_
  ...
  class Raytracer {
    ...
    Brick *addBrickAtIndex(float x, float y, float z, unsigned int w, unsigned int h, unsigned d) {
      glm::vec3 pos(x*w + w/2.0f, y*h + h / 2.0f, z*d + d / 2.0f);
      glm::uvec3 dims(w, h, d);

      Brick *brick = new Brick(pos, dims);
    ...
    void upload() {
      ...
      */
      ...
      glVertexAttribLPointer(1, 1, GL_UNSIGNED_INT64_ARB, 0, 0); gl_error();
      glVertexAttribDivisor(1, 1); gl_error();
