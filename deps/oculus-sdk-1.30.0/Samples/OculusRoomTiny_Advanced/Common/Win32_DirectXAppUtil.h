...
//------------------------------------------------------------
// Simple deCompressed .tag image file loader
// Reference to Texture* LoadTextureTgaEitherWay() in \CommonSrc\Render\Render_LoadTextureTGA.cpp
bool LoadTextureTgaEitherWay(
    const char* filename,
    uint32_t alpha,
    int &out_width,
    int &out_height,
    uint32_t* &out_pix)
{
  OutputDebugStringA("---------------------- Loading .tga file: ");
  OutputDebugStringA(filename);
  OutputDebugStringA("\n");

  std::fstream hFile(filename, std::ios::in | std::ios::binary);
  if (!(hFile.is_open())) {
    OutputDebugStringA("---------------------- Failed open file.\n");
    return false;
  }

  uint8_t Header[18] = {0};
  uint8_t DeCompressed[12] = {0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};

  hFile.read(reinterpret_cast<char*>(&Header), sizeof(Header));

  if (!std::memcmp(DeCompressed, &Header, sizeof(DeCompressed))) {
    int bitsPerPixel = Header[16];
    out_width = Header[13] * 256 + Header[12];
    out_height = Header[15] * 256 + Header[14];
    int size = out_width * out_height; // image size
    out_pix = new uint32_t[size]; // ARGB

    if (bitsPerPixel == 32)
    {
      hFile.read(reinterpret_cast<char*>(out_pix), size * 4);
      return true;
    }
    else if (bitsPerPixel == 24)
    {
      uint8_t* buf = new uint8_t[size * 3]; // RGB
      hFile.read(reinterpret_cast<char*>(buf), size * 3);

      //for (int y = out_height - 1; y >= 0; y--) {
      for (int y = 0; y < out_height; y++) {
        for (int x = 0; x < out_width; x++) {
          uint32_t red   = buf[y * out_width * 3 + x * 3 + 0];
          uint32_t green = buf[y * out_width * 3 + x * 3 + 1];
          uint32_t blue  = buf[y * out_width * 3 + x * 3 + 2];
          out_pix[y * out_width + x] = (red << 16) + (green << 8) + blue + (alpha << 24);   // ARGB
        }
      }
  ...
  else
  {
    hFile.close();
    OutputDebugStringA(
        "---------------------- Invalid File Format. Required: DeCompressed Image.\n");
    return false;
  }
}
...
struct TriangleSet
{
    ...
    void Empty() { numVertices = numIndices = 0; }
...
struct Model
{
    ...
    XMFLOAT3     Pos;
    XMFLOAT4     Rot;
    bool         IsSelfRote;    // self-rotate around a center
    XMFLOAT3     Center;
    XMFLOAT3     MCenter;
    XMFLOAT4     SelfRot;
    Material   * Fill;
    ...
    Model() : Scale(XMFLOAT3(1, 1, 1)), Fill(nullptr), VertexBuffer(nullptr), IndexBuffer(nullptr), IsShown(true), IsSelfRote(false) {};
    ...
    Model(
        ...
        XMFLOAT4 argRot,
        Material* argFill,
        XMFLOAT3 argScale = XMFLOAT3(1, 1, 1))
        : Scale(argScale),
        Pos(argPos),
        Rot(argRot),
        Fill(argFill),
    ...
    void Render(XMMATRIX * projView, float R, float G, float B, float A, bool standardUniforms)
    {
      if (IsShown) {
        ...
        if (IsSelfRote) {
          ...
          modelMat = XMMatrixMultiply(modelMat, XMMatrixTranslationFromVector(XMLoadFloat3(&MCenter)));
          modelMat = XMMatrixMultiply(modelMat, XMMatrixRotationQuaternion(XMLoadFloat4(&SelfRot)));
          // translate back
          modelMat = XMMatrixMultiply(modelMat,
              XMMatrixTranslationFromVector(XMLoadFloat3(&Center)));
        }
        modelMat = XMMatrixMultiply(modelMat, XMMatrixScalingFromVector(XMLoadFloat3(&Scale)));
        modelMat = XMMatrixMultiply(modelMat, XMMatrixRotationQuaternion(XMLoadFloat4(&Rot)));  // rotate around world center
        modelMat = XMMatrixMultiply(modelMat, XMMatrixTranslationFromVector(XMLoadFloat3(&Pos)));
    ...
    // Replace vertex buffer for animation. Must be equal number of vertices.
    bool ReplaceVertexBuffer(const Vertex* newVerts, size_t newSize)
    {
      if (VertexBuffer->Size != newSize * sizeof(Vertex)) {
        OutputDebugStringA("---------------------- Failed replacing vertex buffer : size does not match.");
            return false;
        }

        D3D11_MAPPED_SUBRESOURCE map;
        DIRECTX.Context->Map(VertexBuffer->D3DBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &map);
        memcpy(map.pData, newVerts, newSize * sizeof(Vertex));
...
//---------------------------------------------------------------
// Simple .obj model file loader:
// - only deal with OBJ files with 1 UV coordinate and 1 normal per vertex, triangular mesh
// - output a non-indexed mesh (triangle soup)
bool LoadOBJ(
    const char* filename,
...
{
  OutputDebugStringA("---------------------- Loading OBJ file: ");
  OutputDebugStringA(filename);
  OutputDebugStringA("\n");
  ...
  if (!fp || err) {
    OutputDebugStringA("---------------------- Failed opening the file. Please check the file path.\n");
    getchar();
    return false;
  }
  ...
  int matches = 0;
  for(;;) {
    char lineHeader[256]; // NOTE: assume that the first word of a line won't be longer than 256
    ...
    {
      matches = fscanf_s(fp, "%f %f %f\n", &vertex.x, &vertex.y, &vertex.z);
      if (matches != 3) {
        OutputDebugStringA("---------------------- Failed parsing a vertex.\n");
        fclose(fp);
        return false;
      }
    ...
    {
      matches = fscanf_s(fp, "%f %f\n", &uv.x, &uv.y);
      if (matches != 2) {
        OutputDebugStringA("---------------------- Failed parsing a vertex texture coordinate.\n");
        fclose(fp);
        return false;
      }
    ...
    } else if (strcmp(lineHeader, "vn") == 0) // vertex normal
    {
      ...
      if (matches != 3) {
        OutputDebugStringA("---------------------- Failed parsing a vertex normal.\n");
        fclose(fp);
        return false;
      }
      temp_normals.push_back(normal);
    } else if (strcmp(lineHeader, "f") == 0) // triangular face : C++ indexing starts at 0 and OBJ
                                             // indexing starts at 1
    {
      matches = fscanf_s(
          ...
          &normalIndex[2]);
      if (matches != 9) {
        OutputDebugStringA("---------------------- Failed parsing a triangular face.\n");
        fclose(fp);
        return false;
      }
  ...
  for (unsigned int i = 0; i < vertexIndices.size(); i++) {
    ...
    uv = temp_uvs[uvIndices[i]];
    normal = temp_normals[normalIndices[i]];
