...
public:
    ...
    bool InitDevice(const char* appName, int vpW, int vpH, const VrApi& vrApi, bool windowed = true)
    {
        ...
        debugInfo.flags = VK_DEBUG_REPORT_FLAG_BITS_MAX_ENUM_EXT; // VK_DEBUG_REPORT_ERROR_BIT_EXT | VK_DEBUG_REPORT_WARNING_BIT_EXT;
        ...
        if (!vrApi.GetDeviceExtensionsVk(extensionNames, &extensionNamesSize))
        {
            ...
            // Enable VK_KHR_maintenance1 so we can flip the viewport to get GL NDC
...
public:
    ...
    bool RenderPass::Create(VkFormat aColorFmt, VkFormat aDepthFmt)
    {
        colorFmt = aColorFmt;
        depthFmt = aDepthFmt;
