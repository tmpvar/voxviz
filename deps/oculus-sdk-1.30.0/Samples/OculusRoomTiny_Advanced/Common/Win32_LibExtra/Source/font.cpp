...
Font::Font(char * textureFilename, char * filename, bool isItBlocky)
{
    ...
    while(true)
    {
        u_char this_char;    int first_pixel, last_pixel;
        fscanf_s(file_ptr, "%c %d %d\n", &this_char, 65000, &first_pixel, &last_pixel);
        if (this_char == (u_char)'¬') this_char = ' '; // Special character for space
        if (this_char == (u_char)'[') this_char = GapBetweenChars;
        if (this_char == (u_char)'^') break;
