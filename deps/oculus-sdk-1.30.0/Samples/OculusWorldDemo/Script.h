...
//    Command           args                                                       Notes
//    -----------------------------------------------------------------------------------------------------------------------
//    ExecuteMenu       <menu path string> <menu value>                            Executes a menu item by string path. Quoted strings as-needed.
...
// "ExecuteMenu Scene.StartTiming.anything" will exeucte the Scene.StartTiming trigger.
// 
...
public:
   ...
   ~OWDScript() = default;
    ...
    // function periodicallly. At least once per frame.
    State Step();
...
protected:
    ...
    struct CommandWait : public Command {
        CommandWait() : Command{CommandType::Wait}, WaitAmount(0.0), WaitUnits(none), CompletionValue(0.0){}
        double WaitAmount; // Relative amount.
        enum { none, ms, s, frames } WaitUnits;
