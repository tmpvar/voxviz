...
OculusWorldDemoApp::OculusWorldDemoApp() :
    ...
    GridMode(Grid_Lens),
    GridColorWhite(0), // default to green
    ...
    MenuIsTranslating(false)
{
    ...
    if (!EnvErr && EnvDefaultSceneRenderMode[0])
    {
        ...
        if (EnvRenderModeAsLong < (unsigned long)Scene_Count)
        {
        ...
        else
        {
            WriteLog("[OculusWorldDemoApp] Warning: Unable to set OWD_DEFAULT_SCENE_RENDER_MODE to %i. "
                            ...
                            EnvRenderModeAsLong, (unsigned long)Scene_Count);
        }
    }
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        if (!OVR_stricmp(argStrClean, "scriptfile"))
        {
            if ( i <= argc - 1 ) // next arg is the script file path
            {
                ...
                if (!result)
                    WriteLog("Failed to load script %s.", argv[ i + 1 ]);
...
void OculusWorldDemoApp::PopulateOptionMenu()
{
    ...
    */
    ...
    Menu.AddEnum("Scene Content.Grid Color", &GridColorWhite).
      AddEnumValue("Green", 0).
      AddEnumValue("White", 1);
...
void OculusWorldDemoApp::TraceAllMenuOptions(OptionVar*)
{
    ...
    WriteLog("*** Begin menu item trace ***\n"
             "%s"
    ...
    if (file == nullptr)
        WriteLog("Failed to write menu trace to file: %s\n", fileName);
    else
    {
      ...
      if (FovStencilType >= 0)
      {
        ...
        if (stencilMesh.UsedVertexCount > 0 && error == ovrSuccess)
        {
          ...
          else
          {
            ...
            FovStencilMeshes[curEye] = *new Model(Prim_Triangles, "ViewStencilMesh");
            ...
            {
              FovStencilMeshes[curEye]->AddVertex(stencilMesh.VertexBuffer[curVertIdx].x,
                stencilMesh.VertexBuffer[curVertIdx].y,
                0.0f, vertColor, 0.0f, 0.0f);
            }
            ...
            for (int triIdx = 0; triIdx < numTris; ++triIdx)
            {
              ...
              FovStencilMeshes[curEye]->AddTriangle((uint16_t)stencilMesh.IndexBuffer[curIndex],
                (uint16_t)stencilMesh.IndexBuffer[curIndex + 1],
                (uint16_t)stencilMesh.IndexBuffer[curIndex + 2]);
            }

            FovStencilMeshes[curEye]->Fill = pRender->GetSimpleFill();
          }
...
{
    WriteLog("Exiting with code %d", exitCode);
        ...
        for (int layerNum = 0; layerNum < CamLayerCount; layerNum++)
        {
            ...
            for (int eye = 0; eye < 2; eye++)
            {
                ...
                EyeLayer[layerNum].EyeFov.RenderPose[eye]     = (EnableTimewarpOnMainLayer) ? 
                                                                CamRenderPose[camNum] : // Where the eye was when we asked the SDK earlier.
                                                                Posef::Identity();
...
void OculusWorldDemoApp::RenderEyeView(CamRenderPoseEnum camNum, ovrEyeType eyeNum, const Matrix4f* optionalMatrix, bool onlyRenderWorld)
{
    ...
    if ( (GridDisplayMode != GridDisplay_GridOnly) && (GridDisplayMode != GridDisplay_GridDirect) )
    {
        if (SceneMode != Scene_OculusCubes && SceneMode != Scene_DistortTune)
        {
            ...
            {
              OVR_ASSERT ( !"fix me" );
				      PositionalTracker.Draw(Session, pRender, ThePlayer, TrackingOriginType, Sitting, ExtraSittingAltitude, CamFromWorld, eyeNum, EyeRenderPose);
            }
    		}
        if(!onlyRenderWorld)
        {
