...
void OculusWorldDemoApp::RenderController(CamRenderPoseEnum camNum, const ovrPosef pose, const Vector4f color) {

  pRender->SetCullMode(RenderDevice::Cull_Off);

  Matrix4f scaleUp = Matrix4f::Scaling(1.0f);
  pRender->SetGlobalTint(color);
...
void OculusWorldDemoApp::RenderControllers(CamRenderPoseEnum camNum, ovrEyeType eyeNum )
{
...

    {
      if (ConnectedControllerTypes & ovrControllerType_LTouch) {
        RenderController(camNum, HandPoses[ovrHand_Left], Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
      }
      if (ConnectedControllerTypes & ovrControllerType_RTouch) {
        RenderController(camNum, HandPoses[ovrHand_Right], Vector4f(1.0f, 1.0f, 1.0f, 1.0f));
      }
    }
