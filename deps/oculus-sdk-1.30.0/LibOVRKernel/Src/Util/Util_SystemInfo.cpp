...
String GetOVRPath(const wchar_t* subPath, bool create_dir) {
  return String(GetOVRPathWide(subPath, create_dir).c_str());
}

//-----------------------------------------------------------------------------
// Get a path under BaseOVRPath
// This version returns a std::wstring object.
std::wstring GetOVRPathWide(const wchar_t* subPath, bool create_dir) {
#if defined(_WIN32)
...
// Usually this returns C:\Program Files\Oculus\Support\oculus-runtime
bool GetOVRRuntimePathW(wchar_t runtimePath[MAX_PATH]) {
  ...
  if (DefaultSettingsManager.ReadValue(
    ...
    return (OVR_strlcpy(runtimePath, result.c_str(), MAX_PATH) < MAX_PATH);
  }
